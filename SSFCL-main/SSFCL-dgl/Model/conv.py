...
class DropLearner(nn.Module):
    ...
    def forward(self, node_emb, graph, temperature = 0.5, relation_emb = None, edge_type = None):
        if self.concat:
            ...
            filtered_w_src = filtered_x1 * 0.5 + filtered_x11
            filtered_w_src = np.float32(filtered_w_src)
            w_src = torch.tensor(filtered_w_src)
            w_src = w_src.cuda()
            # MLP
            ...
            filtered_w_dst = filtered_x2 * 0.5 + filtered_x22
            filtered_w_dst = np.float32(filtered_w_dst)
            w_dst = torch.tensor(filtered_w_dst)
            w_dst = w_dst.cuda()
            # MLP
